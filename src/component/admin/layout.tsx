...
const MobileMenu = ({ sidebarOpen, setSidebarOpen, locationPath }: MobileMenuProps) => (
              ...
              <div className="space-y-1 px-2">
                {navigation.map((item) => (
                  <Link
                    ...
                    aria-current={locationPath === item.href ? 'page' : undefined}
                    onClick={() => setSidebarOpen(false)}
                  >
...
const DesktopMenu = ({ locationPath }: DesktopMenuProps) => (
        ...
        <div className="space-y-1 px-2">
          {navigation.map((item) => (
            <Link
              ...
              to={item.href}
              className={classNames(
                locationPath === item.href ? 'bg-sky-800 text-white' : 'text-sky-100 hover:bg-sky-600 hover:text-white',
...
const SideBar = ({ sidebarOpen, setSidebarOpen }: SideBarProps) => {
  ...
  return (
    <>
      <MobileMenu sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} locationPath={location.pathname} />
      <DesktopMenu locationPath={location.pathname} />
    </>
  );
};
